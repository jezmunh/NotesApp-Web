...
<template>
  <div class="flex flex-col min-h-screen min-w-screen">
    ...
    <nav class="flex flex-wrap items-center justify-between mx-auto p-10 w-dvw">
      <p class="font-semibold text-3xl text-white">{{ $t("note_edit.header")}}</p>
      <RouterLink to="/" class="text-sky-500 no-underline hover:underline">{{ $t("note_info.back_link")}}</RouterLink>
    </nav>

    <form @submit.prevent class="lg:max-w-lg md:max-w-sm mx-auto p-10 w-dvw">
        <div class="mb-5">
          <label for="title_note" class="block mb-2 lg:text-2xl md:text-sm font-medium text-gray-900 dark:text-white">{{ $t("note_info.note_heading")}}</label>
        ...
        <div class="mb-5">
            <label for="note_text" class="block mb-2 lg:text-2xl md:text-sm font-medium text-gray-900 dark:text-white">{{ $t("note_info.note_content")}}</label>
        ...
        <button @click="editNoteClient" class="text-white bg-sky-600 hover:bg-sky-900 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full md:w-auto px-5 py-2.5 text-center dark:bg-sky-600 dark:hover:bg-sky-700 dark:focus:ring-slate-800">{{ $t("note_edit.edit_button")}}</button>
        <button @click="deleteNoteClient" class="text-white bg-red-600 hover:bg-red-900 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm w-full md:w-auto px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-slate-800 mx-4">{{ $t("note_edit.delete_button")}}</button>
      </form>
  </div>
