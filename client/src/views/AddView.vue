...
<template>
  <div class="flex flex-col min-h-screen min-w-screen">
    ...
    <nav class="flex flex-wrap items-center justify-between mx-auto p-10 w-dvw">
      <p class="font-semibold text-3xl text-white">{{ $t("note_info.header")}}</p>
    ...
    <form @submit.prevent class="lg:max-w-lg md:max-w-sm mx-auto p-10 w-dvw">
        ...
        <button @click="newNoteClient" class="text-white bg-sky-600 hover:bg-sky-900 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full md:w-auto px-5 py-2.5 text-center dark:bg-sky-600 dark:hover:bg-sky-700 dark:focus:ring-slate-800">{{ $t("note_info.button_add")}}</button>
      </form>
  </div>
